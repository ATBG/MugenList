...
def resolve_anikai():
    ...
    logger.info('Resolving AniKai for: %s, season: %s, episode: %s', title, season, episode)
    ...
    try:
        ...
        result = resolve_anikai_watch(title, episode=episode, season=season, total_episodes=total_episodes, alt_titles=alt_titles)
        duration = time.time() - start_time
        
        confidence = result.get('confidence', 0)
        logger.info('AniKai resolution: title="%s", conf=%s%%, time=%.2fs, url=%s', 
                    title, confidence, duration, result.get('url', 'N/A'))
        
        return jsonify(result)
    except requests.exceptions.Timeout:
        logger.error('AniKai resolution timed out')
        return jsonify({'error': 'Request timed out. AniKai may be slow or unavailable.'}), 504
    except requests.exceptions.ConnectionError:
        logger.error('AniKai resolution connection failed')
        return jsonify({'error': 'Connection failed. Check internet connection.'}), 503
    except Exception as e:
        logger.error('Anikai resolution failed: %s', e)
        return jsonify({'error': str(e)}), 500
...
def scan_local():
    ...
    try:
        ...
        logger.info('Scanned directory: %s, found %d files', path, len(files))
        return jsonify({'files': files, 'count': len(files), 'path': path})
...
PORT = int(os.environ.get('MUGELLIST_PORT', 8000))
