...
class LocalFileResolver:
    ...
    def scan_directory(self, path: str, mal_id: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        for root, _, files in os.walk(path):
            # Check if directory name itself contains season information (e.g. Season 2 or S3)
            dir_season = None
            dir_match = re.search(r'Season\s*(\d+)|\bS(\d+)\b', os.path.basename(root), re.I)
            if dir_match:
                dir_season = int(dir_match.group(1) or dir_match.group(2))

            for f in files:
                # Filter by video extensions
                if f.lower().endswith(('.mkv', '.mp4', '.avi', '.mov', '.flv', '.webm')):
                    # Skip noise/meta files
                    if re.search(EXCLUDE, f, re.I): continue
                    
                    parsed = self.parse_filename(f)
                    if parsed:
                        # Override season if filename has Season 1 (default) but directory specifies another season
                        if parsed.get('season', 1) == 1 and dir_season is not None:
