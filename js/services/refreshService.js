 ...
 * - Check each anime once per 24 hours for metadata updates
 * - Detect changes in episode count, status, airing state
 * - Only update state if changes are found
 * - Emit notifications for user-visible changes
 * - Handle One Piece special case with AniList fallback
 * - Prevent UI blocking and excessive rerenders
 */
...
async function runRefreshCycle() {
  ...
  try {
    ...
    if (refreshedCount > 0) {
      console.log(`✨ Refreshed ${refreshedCount} anime from Jikan`);
    }
  } catch (err) {
