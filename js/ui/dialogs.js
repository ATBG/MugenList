...
import { updateAnimeField, updateSeasonField, deleteAnimeEntry, addAnimeFromBackend } from '../services/animeManager.js';
    ...
    <div class="modal-body">
      <div style="display:flex;gap:16px;margin-bottom:16px;">
        ...
        <div>
          ...
          <div style="font-size:0.8rem;color:var(--text-muted);margin-bottom:4px;"><strong style="color:var(--neon-green)">${statusLabel(normalizeStatus(season.watch_status), season.status === 'Currently Airing' || season.is_airing)}</strong></div>
...
// ---------- Backend-Driven Add Anime Flow ----------

let _searchDebounce = null;

/**
 * Opens the backend-driven search dialog.
 * User types a title → backend searches Jikan → results shown → user selects.
 */
export function openBackendSearchDialog() {
  openModal(`
    <div class="modal-header">
      <h2 class="modal-title">Add Anime</h2>
      <button class="modal-close" id="modal-close-btn" aria-label="Close">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
        </svg>
    ...
    <div class="modal-body">
      <div class="form-group">
        ...
        <input class="form-input" id="backend-search-input" placeholder="e.g. Attack on Titan" autofocus />
      </div>
      <div id="backend-search-status" style="font-size:0.85rem;color:var(--text-muted);margin-bottom:12px;"></div>
      <div id="backend-search-results" class="search-results-list"></div>
    </div>
    <div class="modal-footer">
      <button class="btn btn--secondary" id="modal-cancel-btn">Cancel</button>
    </div>
  ...
  const input = document.getElementById('backend-search-input');
  input?.addEventListener('input', () => {
    clearTimeout(_searchDebounce);
    _searchDebounce = setTimeout(() => _doBackendSearch(input.value), 400);
  });
...
async function _doBackendSearch(query) {
  const statusEl = document.getElementById('backend-search-status');
  const resultsEl = document.getElementById('backend-search-results');
  if (!resultsEl) return;
  ...
  try {
    const data = await backendSearch(query.trim(), 20);
    ...
    resultsEl.innerHTML = results.map((r, i) => `
      <div class="search-result-item" data-idx="${i}" data-mal-id="${r.mal_id}">
        <div class="search-result-poster">
          <img src="${r.poster || 'assets/icons/placeholder.svg'}" alt="" loading="lazy" onerror="this.src='assets/icons/placeholder.svg'" />
        </div>
        <div class="search-result-info">
          <div class="search-result-title">${r.title}</div>
          <div class="search-result-meta">
            <span class="search-meta-badge">${r.type || 'TV'}</span>
            ${r.year ? `<span class="search-meta-badge">${r.year}</span>` : ''}
            ${r.episodes ? `<span class="search-meta-badge">${r.episodes} eps</span>` : ''}
            ${r.score ? `<span class="search-meta-badge">★ ${r.score}</span>` : ''}
            ${r.airing ? '<span class="search-meta-badge search-meta--airing">Airing</span>' : ''}
          </div>
          ${r.genres?.length ? `<div class="search-result-genres">${r.genres.slice(0, 4).map(g => `<span class="genre-tag genre-tag--sm">${g}</span>`).join('')}</div>` : ''}
        </div>
      </div>
    `).join('');
    ...
    resultsEl.querySelectorAll('.search-result-item').forEach(item => {
      item.addEventListener('click', () => {
        ...
        if (selected) {
          closeModal();
          _onSearchResultSelected(selected);
        }
      });
    });
    ...
    if (statusEl) statusEl.textContent = 'Search failed — is the backend running?';
  }
}

async function _onSearchResultSelected(result) {
  showToast('Loading franchise data...', 'info');

  try {
    const relData = await backendGetRelations(result.mal_id);

    if (relData.relations && relData.relations.length > 0) {
      // Show the franchise selection dialog
      openBackendRelationDialog(result, relData);
    } else {
      // No relations found — save just this one entry
      await _saveAndAddEntry(result.mal_id, []);
    }
  } catch (err) {
    console.error('Franchise relations fetch failed:', err);
    // Save just the main entry as fallback
    await _saveAndAddEntry(result.mal_id, []);
  }
}

/**
 * Opens the franchise relation selection dialog (backend-driven).
 */
function openBackendRelationDialog(mainResult, relData) {
  const relations = relData.relations || [];

  const relationHtml = relations.map((rel, i) => `
    <div class="relation-candidate" data-index="${i}">
      <div class="relation-candidate__info">
        <div class="relation-candidate__title">${rel.title || 'Unknown'}</div>
        <div class="relation-candidate__meta">
          <span class="relation-type-badge">${rel.relationType}</span>
          <span class="search-meta-badge">${rel.format || 'TV'}</span>
          ${rel.episodes ? `<span class="search-meta-badge">${rel.episodes} eps</span>` : ''}
        </div>
      </div>
      <div class="relation-candidate__checkbox">
        <input type="checkbox" id="rel-chk-${i}" ${rel.prechecked ? 'checked' : ''} />
      </div>
    </div>
  `).join('');
  ...
  openModal(`
    <div class="modal-header">
      <h2 class="modal-title">Franchise for "${mainResult.title}"</h2>
      <button class="modal-close" id="modal-close-btn" aria-label="Close">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
        </svg>
    ...
    <div class="modal-body">
      <p style="color:var(--text-secondary);font-size:0.9rem;margin-bottom:16px;">
        Select which related titles to include in this franchise:
      </p>
      <div class="relation-candidates-list">
        ${relationHtml}
      </div>
    ...
    <div class="modal-footer">
      <button class="btn btn--secondary" id="modal-skip-btn">Just Add Main</button>
      <button class="btn btn--primary" id="modal-add-btn">Add Selected</button>
    </div>
  `, true);
  ...
  document.getElementById('modal-skip-btn')?.addEventListener('click', async () => {
    closeModal();
    await _saveAndAddEntry(mainResult.mal_id, []);
  });

  document.getElementById('modal-add-btn')?.addEventListener('click', async () => {
    const selectedIds = [];
    relations.forEach((rel, i) => {
      const chk = document.getElementById(`rel-chk-${i}`);
      if (chk?.checked && rel.mal_id) {
        selectedIds.push(rel.mal_id);
      }
    });
    closeModal();
    await _saveAndAddEntry(mainResult.mal_id, selectedIds);
  });
...
async function _saveAndAddEntry(mainMalId, selectedRelationIds) {
  showToast('Fetching full metadata...', 'info');

  try {
    const entry = await backendSaveFranchise(mainMalId, selectedRelationIds);
    await addAnimeFromBackend(entry);
  } catch (err) {
    console.error('Failed to save franchise bundle:', err);
    showToast('Failed to add anime. Check backend logs.', 'error');
  }
}
